...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, deblur : int):
    ...
    def get_features(self):
        ...
        return torch.cat((features_dc, features_rest), dim=1), torch.cat((features_dc_color, features_rest), dim=1)
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        # features[:, :3, 0 ] = torch.ones_like(fused_color) * 0.5
        fused_color = (0.2990 * fused_color[:, 0:1] + 0.5870 * fused_color[:, 1:2] + 0.1140 * fused_color[:, 2:3]).repeat(1, 3)
        ...
        self._features_dc_color = nn.Parameter(features[:, :, 0:1].transpose(1, 2).contiguous().requires_grad_(True))
    ...
    def save_ply(self, path):
        ...
        attributes = np.concatenate((xyz, normals, f_dc, f_dc_color, f_rest, opacities, scale, rotation), axis=1)
    ...
    def load_ply(self, path):
        ...
        rot_names = [p.name for p in plydata.elements[0].properties if (p.name.startswith("rot") and len(p.name.split("_"))== 2)]
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_dc_color, new_features_rest, new_opacities, new_scaling, new_rotation):
        d = {"xyz": new_xyz,
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_features_dc_color, new_features_rest, new_opacity, new_scaling, new_rotation)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_features_dc_color, new_features_rest, new_opacities, new_scaling, new_rotation)
    ...
    def add_densification_stats(self, viewspace_point_tensor, update_filter, d=1.0):
        if type(viewspace_point_tensor) == list:
            for i in range(len(viewspace_point_tensor)):
                self.xyz_gradient_accum[update_filter[i]] += torch.norm(viewspace_point_tensor[i].grad[update_filter[i],:2], dim=-1, keepdim=True)
                self.denom[update_filter[i]] += d
        else:
            self.xyz_gradient_accum[update_filter] += torch.norm(viewspace_point_tensor.grad[update_filter,:2], dim=-1, keepdim=True)
            self.denom[update_filter] += d

    def add_points(self, training_args, dist=10000.0, N=4, num_pts=100_000, bound=50):
        print("Create additional points (will take about 1 min)")
        existing_points = self.get_xyz
        existing_color = self.get_features[1].transpose(1, 2).view(-1, 3, (self.max_sh_degree+1)**2)
        min_dist = torch.tensor([dist], device=existing_points.device)
        num_pts = num_pts
        existing_points_sort = existing_points.sort(0)[0]
        bbox_min = existing_points_sort[bound]
        bbox_max = existing_points_sort[-bound]

        additional_points = torch.rand((num_pts, 3), device=existing_points.device)
        additional_points = additional_points * (bbox_max - bbox_min) + bbox_min
        dummy_color = torch.rand((3, (self.max_sh_degree+1)**2), device=existing_points.device)
        mask_pts = torch.ones(num_pts, dtype=torch.bool, device=existing_points.device)
        ...
        l = [
            ...
            {'params': [self._features_rest], 'lr': training_args.feature_lr / 20.0, "name": "f_rest"},
            {'params': [self._opacity], 'lr': training_args.opacity_lr, "name": "opacity"},
            {'params': [self._scaling], 'lr': training_args.scaling_lr * 1, "name": "scaling"},
            {'params': [self._rotation], 'lr': training_args.rotation_lr * 1, "name": "rotation"}
        ]
