...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier=1.0, deblur=0, use_pos=False,
           lambda_s=0.01, lambda_p=0.01, max_clamp=1.1, ifcolor=True):
    """
    Render the scene. 
    ...
    if pipe.compute_cov3D_python:
        ...
        if not deblur:  # testing time
            ...
            rendered_image = rendered_image.clamp(0, 5)
            rendered_image = 1 - torch.exp(-rendered_image)

            rendered_image_color = rendered_image_color.clamp(0, 5)
            rendered_image_color = 1 - torch.exp(-rendered_image_color)

            return {"render": rendered_image,
            "render_color": rendered_image_color,
            ...
            if not use_pos:    # defocus 
                ...
                renders_color = [rendered_image_color]
                viewspace_points = [screenspace_points]
                ...
                for i in range(M):
                    ...
                    # rendered_image, _radii, _ = rasterizer(
                    ...
                    renders_color.append(rendered_image_color_)
                    viewspace_points.append(screenspace_points)
                    visibility_filter.append(_radii > 0)
                    radii.append(_radii)

                rendered_image_color = sum(renders_color) / len(renders_color)
